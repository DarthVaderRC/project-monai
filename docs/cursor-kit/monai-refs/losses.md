...
Named neighbor for the demo: **`DiceLoss`** in `monai/losses/dice.py`. Reference implementation shipped by this kit: **`LogCoshDiceLoss`** (same file) — a `DiceLoss` subclass whose `forward` wraps the result in `log(cosh(...))`.
