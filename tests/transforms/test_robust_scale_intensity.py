...
"""Tests for RobustScaleIntensity (issue #1)."""
...
class TestRobustScaleIntensity(unittest.TestCase):
    ...
    def test_default_iqr(self, p):
        """Happy path: median ≈ 0 after scale; IQR-based spread."""
        result = RobustScaleIntensity()(p(IMG))
        expected = p(_expected(IMG, 25.0, 75.0))
        assert_allclose(result, expected, type_test="tensor", rtol=1e-4, atol=1e-4)
        flat = np.asarray(result).astype(np.float64).ravel()
        self.assertTrue(np.isfinite(flat).all())
        self.assertAlmostEqual(float(np.median(flat)), 0.0, places=4)
    ...
    def test_dtype(self, p):
        result = RobustScaleIntensity(dtype=np.float64)(p(IMG))
    ...
    def test_default_dtype(self):
        """Default dtype must be np.float32 (not removed numpy aliases)."""
        self.assertEqual(RobustScaleIntensity().dtype, np.float32)
